...
def train_fold(config, fold_idx):
    ...
    wandb.init(project="TaikoNation-Style-Conditioning", config=config, name=run_name, reinit=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"--- Starting Fold {fold_idx + 1} on {device} ---")

    train_loader, val_loader, tokenizer, mapper_vocab = get_transformer_data_loaders(config, fold_idx)
    if not train_loader:
        wandb.finish()
        return
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            output = model(encoder_input, decoder_input, mapper_id)
    ...
    mapper_vocab_path = os.path.join(os.path.dirname(model_save_path), "mapper_vocab.json")
    with open(mapper_vocab_path, 'w') as f:
        json.dump(mapper_vocab, f)
    print(f"Mapper vocabulary saved to {mapper_vocab_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Taiko Transformer model with style conditioning.")
